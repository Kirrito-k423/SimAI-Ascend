...
uint64_t FlatRandomMultiplier(uint64_t active_ranks, uint64_t seed) {
  ...
  while (GreatestCommonDivisor(multiplier, active_ranks) != 1U) {
    multiplier += 2U;
...
int PhysicalRank(
  ...
  const uint64_t multiplier = FlatRandomMultiplier(ranks, seed);
...
bool ValidateConfig(
  ...
  if (!CheckedPositiveProduct3(
      ...
      summary->active_ranks % attention_denominator != 0) {
    return Fail(summary, "ATTENTION_GRID_NOT_DIVISIBLE");
  }
  ...
  for (const int ep : config.ep_values) {
    ...
    const int ep_remainder = summary->active_ranks % ep;
    if (config.message_bytes_per_rank == 0U ||
        config.message_bytes_per_rank % static_cast<uint64_t>(ep) != 0U ||
        (ep_remainder != 0 &&
         config.message_bytes_per_rank %
                 static_cast<uint64_t>(ep_remainder) !=
             0U)) {
...
std::string GroupCanonical(
  ...
  std::ostringstream canonical;
  canonical << "simai.communication-groups/v1|" << grid << "|"
  ...
  if (grid == "ATTENTION") {
    canonical << "|TP=" << config.attention_tp
              ...
              << summary.active_ranks /
                     (static_cast<int64_t>(config.attention_tp) *
                      config.attention_cp *
                      config.attention_pp)
              << "|PP=" << config.attention_pp;
  } else {
    canonical << "|ETP=" << config.moe_etp << "|EP=" << ep
              << "|EDP=floor(N/(ETP*EP*PP))|PP=" << config.moe_pp
              << "|remainder=" << summary.active_ranks % ep;
  }
  canonical << "|logical-contiguous-groups-then-rank-map";
  return canonical.str();
}
...
bool BuildCandidate(
  ...
  candidate->partial_ep_group_ranks = summary->active_ranks % ep;
  candidate->ragged = candidate->partial_ep_group_ranks != 0;
  candidate->full_ep_groups = summary->active_ranks / ep;
  candidate->id = std::string(TopologyPlacementKindName(placement)) +
      "_EP" + std::to_string(ep);
  const std::string attention_group_canonical = GroupCanonical(
      config, *summary, rank_map_digest, placement, ep, "ATTENTION");
  const std::string moe_group_canonical = GroupCanonical(
      config, *summary, rank_map_digest, placement, ep, "MOE");
  TopologyPlacementCommunicationGroup attention_group;
  attention_group.grid = "ATTENTION";
  attention_group.representation = "COMPLETE_CANONICAL_FORMULA_V1";
  attention_group.membership_formula =
      "mixed-radix TP*CP*DP*PP coordinates mapped by rank_map";
  attention_group.membership_digest = digest(attention_group_canonical);
  candidate->communication_groups.push_back(attention_group);
  TopologyPlacementCommunicationGroup moe_group;
  moe_group.grid = "MOE";
  moe_group.representation = "COMPLETE_CANONICAL_FORMULA_V1";
  moe_group.membership_formula =
      "contiguous logical EP groups mapped by rank_map; final group may be ragged";
  moe_group.membership_digest = digest(moe_group_canonical);
  candidate->communication_groups.push_back(moe_group);
  ...
  const int group_count = candidate->full_ep_groups +
      (candidate->partial_ep_group_ranks == 0 ? 0 : 1);
  for (int group = 0; group < group_count; ++group) {
    const int first_logical = group * ep;
    const int group_size = std::min(ep, summary->active_ranks - first_logical);
    ...
    if (!CheckedAdd(projected_summary.global_bytes, &candidate->global_bytes)) {
      return false;
    }
  ...
  candidate_canonical << "simai.topology-placement-candidate/v1|"
                      ...
                      << attention_group.membership_digest << "|"
                      << moe_group.membership_digest;
