...
- 静态检查：`git diff --check`、`bash -n scripts/prototype-ascend-provider-seam.sh`、公开敏感信息扫描。

完整 NS3 最终链接不作为本 prototype 的通过条件：prototype 之前已在本机稳定触发 Upstream macOS arm64 未定义符号；本轮只要求包含改动文件的 `libapplications-obj` 重新编译成功，并显式保留该基线限制。
